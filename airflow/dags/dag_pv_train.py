...
with DAG(
    ...
    api_call = BashOperator(
        task_id="api_call",
        bash_command="cd /opt/airflow/pipeline/src && python -m pv_train.api_pv_train",
    )
    ...
    extract = BashOperator(
        task_id="extract",
        bash_command="cd /opt/airflow/pipeline/src && python -m pv_train.extract_pv_train",
    )
    ...
    data_import = BashOperator(
        task_id="import",
        bash_command="cd /opt/airflow/pipeline/src && python -m pv_train.import_pv_train",
    )
